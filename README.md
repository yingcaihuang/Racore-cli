...
从 [GitHub Releases](https://github.com/yingcaihuang/Racore-cli/releases) 下载最新的 `racore-cli_xxx_windows_amd64.msi`，双击安装。

安装到 `C:\Program Files\Racore CLI\`，**自动添加到系统 PATH**，无需手动配置。
...
racore-cli 支持三种认证方式（按优先级排序）：

**方式 1：环境变量（推荐用于 MCP 配置）**
...
```json
{
  "mcpServers": {
    "racore": {
      "command": "racore-cli",
      "args": ["serve", "--access-key", "your_key", "--secret-key", "your_secret"]
    }
  }
}
```
...
认证信息保存在 `~/.racore/credentials`，之后 `racore-cli serve` 会自动读取。
